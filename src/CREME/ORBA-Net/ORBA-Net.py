...
def load_data(train_path, test_path):
    ...
    network_train  = train_df.iloc[:, 62:166].values
    physical_train = train_df.iloc[:, 0:62].values
    labels_train   = train_df['Label'].values

    network_test   = test_df.iloc[:, 62:166].values
    physical_test  = test_df.iloc[:, 0:62].values
...
class ORBANet(nn.Module):
    ...
    def __init__(self, net_dim, phy_dim, num_classes, hp):
        ...
        self.phy_tok = FeatureTokenizer(phy_dim, d, 4, 0.2)
        self.net_tok = FeatureTokenizer(net_dim, d, 4, 0.2)
...
def class_balanced_weights(labels_np, num_classes, beta=0.9):
...
def train_and_evaluate(data, seed, device, batch_size=32):
    set_seed(seed)
    num_classes  = data['num_classes']
    class_names  = data['class_names']

    test_counts = np.bincount(data['labels_test'].numpy(), minlength=num_classes)
    well_idx    = np.where(test_counts >= 10)[0]
    if len(well_idx) == 0:
    ...
    for epoch in range(hp['num_epochs']):
        ...
        if (epoch + 1) % 10 == 0 or epoch == 0 or epoch == hp['num_epochs'] - 1:
...
# ============================================================================
# Formatting utilities for single-run tables
# ============================================================================
def _vw(s):
...
def print_single_result(result, class_names):
    """Print formatted result tables for a single experimental run, without ±std."""
    name_w, col_w = 38, 18
    cols = ['F1 (Test)', 'Drop-Host F1', 'Drop-Net F1']
    ...
    print("\nNote: 'Drop-Host' and 'Drop-Net' indicate results obtained by "
          "masking the Host and network modalities respectively, "
          "i.e., using only network-layer data and only host-layer data.\n")
...
def run_experiment(train_path, test_path):
    ...
    seed = 1
    print(f"\n--- Running ORBA ---")
    result = train_and_evaluate(data, seed, device, batch_size=32)
